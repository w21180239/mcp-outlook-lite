...
export async function createDraftTool(authManager: any, args: Record<string, any>) {
  ...
  try {
    ...
    if (replyToMessageId) {
      ...
      if (finalBody || (to && to.length > 0) || cc.length > 0 || bcc.length > 0) {
        replyPayload.message = {} as Record<string, any>;
        if (finalBody) {
        ...
        if (to && to.length > 0) {
          replyPayload.message.toRecipients = to.map((email: any) => ({
            emailAddress: { address: email },
          }));
        }
        if (cc.length > 0) {
          replyPayload.message.ccRecipients = cc.map((email: any) => ({
            emailAddress: { address: email },
          }));
        }
        if (bcc.length > 0) {
          replyPayload.message.bccRecipients = bcc.map((email: any) => ({
            emailAddress: { address: email },
          }));
        }
      }
      ...
      if (cc.length > 0) {
        draft.ccRecipients = cc.map((email: any) => ({
          emailAddress: { address: email },
        }));
      }

      if (bcc.length > 0) {
        draft.bccRecipients = bcc.map((email: any) => ({
          emailAddress: { address: email },
        }));
      }

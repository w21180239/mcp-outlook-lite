...
export async function sendEmailTool(authManager: any, args: Record<string, any>) {
  ...
  try {
    ...
    if (cc.length > 0) {
      message.ccRecipients = cc.map((email: any) => ({
        emailAddress: { address: email },
      }));
    }

    if (bcc.length > 0) {
      message.bccRecipients = bcc.map((email: any) => ({
        emailAddress: { address: email },
      }));
    }

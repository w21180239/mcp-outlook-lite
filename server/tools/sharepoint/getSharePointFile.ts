...
function applyDecodedContent(target: Record<string, any>, decodedContent: Record<string, any>, contentType: string | null) {
 ...
 */
function parseSharePointUrl(sharePointUrl: string) {
  try {
 ...
 */
async function resolveSharedFile(graphClient: any, sharingUrl: string, urlInfo: Record<string, any>) {
  const strategies = [
 ...
 */
async function getFileFromDrive(graphClient: any, driveId: string, itemId: string, downloadContent = false) {
  try {
    // Get file metadata
 ...
 */
export async function getSharePointFileTool(authManager: any, args: Record<string, any>) {
  try {
    // Input validation
    ...
    const finalResponse: any = await handleLargeContent(response, 'sharepoint_file', {
 ...
 * @returns {object} MCP tool response
 */
export async function resolveSharePointLinkTool(authManager: any, args: Record<string, any>) {
  try {
    if (!args.sharePointUrl) {
 ...
 * @returns {object} MCP tool response
 */
export async function listSharePointFilesTool(authManager: any, args: Record<string, any>) {
  try {
    await authManager.ensureAuthenticated();
    const graphApiClient = authManager.getGraphApiClient();
